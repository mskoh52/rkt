...
(define %charterm:stty-minus-f-arg-string
  (case (system-type 'os)
    ((macosx) "-f")
    (else     (let ((ostype (getenv "OSTYPE")))
                (if (and (string? ostype)
                         (regexp-match? #rx"^(?i:freebsd|netbsd|openbsd)" ostype))
                    "-f"
                    "-F")))))
...
(define (open-charterm #:tty      (tty      #f)
  ...
  (let* ((tty (cleanse-path (or tty "/dev/tty")))
    ...
    (or (system* (find-executable-path "stty")
                              ...
                              ((string? arg)
                               (string->bytes/utf-8 arg
                                                      ...
                                                      (if truncate
                                                          ...
                                                          (string-length arg))))
                              ((number? arg)
                               (string->bytes/utf-8 (number->string arg)
                                                      %charterm:err-byte))
...
(doc history
     ...
     (#:planet 5:0 #:date "!!!"
               (itemlist
                ...
                (item "Incrementing major version because we're messing with "
                      (code "stty")
