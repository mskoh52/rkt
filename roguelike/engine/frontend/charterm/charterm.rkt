...
terminals.  It does not run on Microsoft Windows, but terminal emulator and Telnet/SSH programs running on Windows can probably use "
  (bold "charterm")
  " programs running on a Unix-like.")
 (para
  "This package could be used to implement a status/management console
...
program.  "
  (bold "charterm")
  " is implemented in pure Racket code except for executing the "
  (code "stty")
  " program for some purposes.  Specifically, "
  (code "stty")
...
(doc
 ...
 (para
  "This demo reports what keys you pressed, while letting you edit a
...
small tests of the "
  ...
  " key.")
 (para
  "Note: Although this demo includes an editable text field, as proof
...
(doc (subsection "Simple Example")
     (para "Here's your first " (bold "charterm") " program:")
     (RACKETBLOCK (UNSYNTAX (code "#lang racket/base"))
                  (require charterm)
...
(doc
 (section "Terminal Diversity")
 (para "Like people, few terminals are exactly the same.")
 (para "Some key (ha) terms (ha) used by " (bold "charterm") " are:")
 (itemlist
  (item (tech "termvar")
        ...
        (tech "keydec")
        ".")
  (item (tech "protocol") " --- how to control the display, query for information, etc.")
  (item
   (tech "keydec")
...
contributing a TeleVideo 950, and Paul McCabe for a Wyse S50 WinTerm.")
 (para
  "At time of this writing, the author is looking to acquire a DEC
VT525, circa 1994, for ongoing testing.")
 (para
  "The author welcomes feedback on useful improvements to "
  (bold "charterm")
  "'s support for terminal diversity.  If you have a terminal that is
sending an escape sequence not recognized by the demo, you can run the demo
with the "
  (Flag "n")
  " (aka "
  (DFlag "no-escape")
...
are for unusual situations.")
  ...
  (item
   (deftech (code "wyse-wy50") " protocol")
...
to run the Wyse terminal in an ANSI or VT100 mode.")
  (item
   (deftech (code "televideo-925") " protocol")
  ...
  (syntax-case stx (else)
    [(_ ERROR-NAME ACTUAL-PROTO (PART0 PART1 PARTn ...) ...)
     (let loop-clauses ([clause-stxes (syntax->list #'((PART0 PART1 PARTn ...) ...))]
                        ...
                        [else-stx #f]
                        [need-protos-hash (make-immutable-hasheq (map (lambda (proto) (cons proto #t))
                                                                      '(ansi televideo-925
                                                                             wyse-wy50)))])
                   ...
                   (let ([actual-proto ACTUAL-PROTO])
                     (case actual-proto
                       ...
                       #,(or else-stx
                             (syntax/loc stx
                               (else
                                (error ERROR-NAME "unimplemented for protocol: ~S" actual-proto)))))))
                 ...
                 (let loop-protos ([proto-stxes (syntax->list (car (syntax->list clause-stx)))]
                   ...
                   (if (null? proto-stxes)
                       (loop-clauses (cdr clause-stxes)
                                     ...
                                     need-protos-hash)
                       (let* ([proto-stx (car proto-stxes)] [proto-e (syntax-e proto-stx)])
                         (if (symbol? proto-e)
...
(define-syntax (%charterm:unimplemented stx)
  (syntax-case stx ()
    [(_ CT ERROR-NAME)
     (syntax/loc stx
       (error ERROR-NAME "unimplemented feature for protocol ~S" (charterm-protocol CT)))]))
...
Googling, a horrifying 2005 Web post by Phil Gregory ["
  (tech "Gregory")
  "] showed that key encoding among XTerm variants was even worse than
one could ever fear.  Even if one already knew how much subtleties of old
terminals varied (e.g., auto-newline behavior, whether an attribute change
consumed a space, etc.), this incompatibility in newer software was
...
(define (charterm-keycode? x)
  (if (or (symbol? x) (char? x) (exact-nonnegative-integer? x)) #t #f))
...
(define-struct charterm-keyinfo (keyset-id bytelang bytelist keylabel keycode all-keycodes)
  #:transparent)

(doc (defproc (charterm-keyinfo? (x any/c)) boolean?)
     "Predicate for whether or not "
     (racket x)
     " is a "
     (racket charterm-keyinfo)
...
(doc (defproc* (((charterm-keyinfo-keyset-id (ki charterm-keyinfo?)) symbol?)
               ...
               (para "Get information from a " (racket charterm-keyinfo) " object.")))
...
(define %charterm:bytestr-to-byte-hash
  (make-hash `(("nul" . 0) ("null" . 0)
                           ...
                           ,@(for/list ([n (in-range 1 26)])
                               (cons (string #\^ (integer->char (+ 96 n))) n))
                           ,@(for/list ([n (in-range 1 26)])
                               (cons (string-append "ctrl-" (string (integer->char (+ 96 n)))) n))
                           ,@(for/list ([n (in-range 32 127)])
                               (cons (string (integer->char n)) n))
                           ,@(for/list ([n (in-range 0 255)])
                               (cons (string-append "(" (number->string n) ")") n)))))
...
(define (%charterm:bytelang->bytelist bytelang secondary?)
  (let ([bytelist (map %charterm:bytestr->byte (regexp-split #rx" +" bytelang))])
    (if (and secondary? (not (= 1 (length bytelist))))
        (error '%charterm:bytelang->bytelist "bytelist for secondary keyset: ~S" bytelist)
        bytelist)))
...
(define (%charterm:keycode->keylabel keycode)
  (cond
    ...
    [else (error '%charterm:keycode->keylabel "invalid keycode: ~S" keycode)]))

(define (%charterm:keylang->keyinfo keyset-id keylang secondary?)
  (apply (lambda (bytelang . args)
           (let-values ([(bytelist) (%charterm:bytelang->bytelist bytelang secondary?)]
                        [(keylabel keycode all-keycodes)
                         (let ([keylabel (car args)])
                           (if (or (string? keylabel) (not keylabel))
                               (values keylabel (cadr args) (cdr args))
                               (let ([keycode (car args)])
                                 (values (%charterm:keycode->keylabel keycode) keycode args))))])
             (make-charterm-keyinfo keyset-id bytelang bytelist keylabel keycode all-keycodes)))
         keylang))
...
(define-struct charterm-keyset (id primary-keyinfos secondary-keyinfos)
  #:transparent)

(doc (defproc (charterm-keyset? (x any/c))
              boolean?
              (para "Predicate for whether or not " (racket x) " is a keyset.")))
(provide charterm-keyset?)

(doc (defproc (charterm-keyset-id (ks charterm-keyset?)) symbol?)
...
(define (make-charterm-keyset-from-keylangs keyset-id keylangs (secondary-keylangs '()))
  (let ([primary-keyinfos (map (lambda (keylang) (%charterm:keylang->keyinfo keyset-id keylang #f))
                               keylangs)]
        [secondary-keyinfos (map (lambda (keylang) (%charterm:keylang->keyinfo keyset-id keylang #t))
                                 secondary-keylangs)])
    ;; (%charterm:assert-keyinfos primary-keyinfos)
    ;; (%charterm:assert-keyinfos secondary-keyinfos)
    (charterm-keyset keyset-id primary-keyinfos secondary-keyinfos)))

(doc (defthing charterm-ascii-keyset
               charterm-keyset?
               (para "From the old ["
                     (tech "ASCII")
  ...
  (let ([keylangs `(("(0)" "NUL" nul null)
                    ...
                    ,@(for/list ([n (in-range 32 127)])
                        (let ([c (integer->char n)])
                          (list (string-append "(" (number->string n) ")") (string c) c))))])
    (make-charterm-keyset-from-keylangs 'ascii keylangs keylangs)))

(doc (defthing
      charterm-dec-vt100-keyset
      charterm-keyset?
      (para
       "From the DEC VT100.  This currently defines the four function
...
(doc (defthing charterm-dec-vt220-keyset
               charterm-keyset?
...
(doc (defthing charterm-screen-keyset
               charterm-keyset?
               (para "From the "
                     (hyperlink "http://en.wikipedia.org/wiki/GNU_Screen" "GNU Screen")
                     " terminal multiplexer, according to ["
                     (tech "Gregory")
                     "].  Also used by "
                     (hyperlink "http://en.wikipedia.org/wiki/Tmux" (code "tmux"))
                     ".")))
(provide charterm-screen-keyset)
(define charterm-screen-keyset
  (make-charterm-keyset-from-keylangs 'screen
...
(doc (defthing charterm-linux-keyset
               charterm-keyset?
...
(define charterm-linux-keyset
  (make-charterm-keyset-from-keylangs
   'linux
   '(("esc [ [ A" f1) ("esc [ [ B" f2) ("esc [ [ C" f3) ("esc [ [ D" f4) ("esc [ [ E" f5))))

(doc (defthing charterm-xterm-x11r6-keyset
               charterm-keyset?
               (para "From the XTerm in X11R6, according to [" (tech "Gregory") "].")))
(provide charterm-xterm-x11r6-keyset)
(define charterm-xterm-x11r6-keyset
  (make-charterm-keyset-from-keylangs 'xterm-x11r6
                                      '(("esc [ 1 1 ~" f1) ("esc [ 1 2 ~" f2)
                                                           ("esc [ 1 3 ~" f3)
                                                           ("esc [ 1 4 ~" f4)
                                                           ("esc [ 1 5 ~" f5)
...
(doc (defthing charterm-xterm-xfree86-keyset
               charterm-keyset?
               (para "From the XFree86 XTerm, according to [" (tech "Gregory") "].")))
(provide charterm-xterm-xfree86-keyset)
(define charterm-xterm-xfree86-keyset
  (make-charterm-keyset-from-keylangs 'xterm-xfree86
                                      '(("esc O P" f1) ("esc O Q" f2)
                                                       ("esc O R" f3)
                                                       ("esc O S" f4)
                                                       ("esc [ 1 5 ~" f5)
...
(doc (defthing charterm-xterm-new-keyset
               charterm-keyset?
...
(doc (defthing charterm-rxvt-keyset
               charterm-keyset?
               (para "From the "
                     (hyperlink "http://en.wikipedia.org/wiki/Rxvt" (code "rxvt"))
...
(define charterm-rxvt-keyset
  (make-charterm-keyset-from-keylangs
   ...
   '(("esc [ 1 1 ~" f1)
     ...
     ("esc [ 2 1 ~" f10)
     ("esc [ 2 3 ~" shift-f1
                    f11) ;; TODO: These shift- and ctrl- are actually from termvar xterm in an rxvt
...
(doc (defthing charterm-wyse-wy50-keyset
               charterm-keyset?
...
(doc (defthing charterm-televideo-925-keyset
               charterm-keyset?
...
(provide charterm-televideo-925-keyset
         charterm-keyset?)
(define charterm-televideo-925-keyset
  (make-charterm-keyset-from-keylangs
   'televideo-925
   '(("ctrl-a @ cr" f1) ("ctrl-a A cr" f2)
                        ("ctrl-a B cr" f3)
                        ("ctrl-a C cr" f4)
                        ("ctrl-a D cr" f5)
                        ("ctrl-a E cr" f6)
...
(define (%charterm:keytree-add-keyinfo-if-can keytree keyinfo)
  (let ([bytelist (charterm-keyinfo-bytelist keyinfo)])
    (let loop-bytelist ([this-byte (car bytelist)] [rest-bytes (cdr bytelist)] [node keytree])
      (cond
        [(hash? node)
         (cond
           [(hash-ref node this-byte #f)
            =>
            (lambda (existing-sub-node)
              ...
              (if (null? rest-bytes)
                  ...
                  (hash-set node
                            this-byte
                            (loop-bytelist (car rest-bytes) (cdr rest-bytes) existing-sub-node))))]
           [else
                      ...
                      (let loop ([rest-bytes rest-bytes])
                        (if (null? rest-bytes)
                            ...
                            (%charterm:make-keytree
                             (cons (cons (car rest-bytes) (loop (cdr rest-bytes))) '())))))])]

        ;; Node is already a keyinfo, so can't add.
        [(charterm-keyinfo? node) node]
        [else
         (error '%charterm:keytree-add-keyinfo-if-can
...
(define (%charterm:keytree-add-any-keyinfos-can keytree keyinfos)
  (let loop ([keyinfos keyinfos] [keytree keytree])
    (if (null? keyinfos)
        keytree
        (loop (cdr keyinfos) (%charterm:keytree-add-keyinfo-if-can keytree (car keyinfos))))))

(define (%charterm:make-keytree-from-keyinfoses keyinfoses)
  (let loop ([keyinfoses keyinfoses] [keytree (%charterm:make-keytree)])
    (if (null? keyinfoses)
        keytree
        (let ([keyinfos (car keyinfoses)])
          ...
          ;;             "bad keyinfos: ~S"
          ;;             keyinfos))
          (loop (cdr keyinfoses) (%charterm:keytree-add-any-keyinfos-can keytree keyinfos))))))

(doc (defproc (charterm-keydec-id (kd charterm-keydec?))
              symbol?
              (para "Gets the ID symbol of the " (tech "keydec") " being used.")))
(provide charterm-keydec-id)

(struct charterm-keydec (id primary-keytree secondary-keytree) #:transparent)

(define (charterm-make-keydec keydec-id . keysets)
  (charterm-keydec
   keydec-id
   (%charterm:make-keytree-from-keyinfoses (map charterm-keyset-primary-keyinfos keysets))
   (%charterm:make-keytree-from-keyinfoses (map charterm-keyset-secondary-keyinfos keysets))))

(doc (subsubsub*section "ANSI Keydecs"))

(doc (defthing charterm-vt100-keydec
               charterm-keydec?
               (para (tech "Keydec") " for " (tech "termvar") " " (racket "vt100") ".")))
...
(doc (defthing charterm-vt220-keydec
               charterm-keydec?
               (para (tech "Keydec") " for " (tech "termvar") " " (racket "vt220") ".")))
...
(doc (defthing charterm-screen-keydec
               charterm-keydec?
               (para (tech "Keydec") " for " (tech "termvar") " " (racket "screen") ".")))
...
(doc (defthing charterm-linux-keydec
               charterm-keydec?
               (para (tech "Keydec") " for " (tech "termvar") " " (racket "linux") ".")))
...
(doc (defthing charterm-xterm-new-keydec
               charterm-keydec?
               (para (tech "Keydec") " for " (tech "termvar") " " (racket "xterm-new") ".")))
...
(doc (defthing charterm-xterm-keydec
               charterm-keydec?
               (para (tech "Keydec")
                     " for "
                     (tech "termvar")
...
(doc (defthing charterm-rxvt-keydec
               charterm-keydec?
               (para (tech "Keydec") " for " (tech "termvar") " " (racket "rxvt") ".")))
...
(doc (defthing charterm-wy50-keydec
               charterm-keydec?
               (para (tech "Keydec") " for " (tech "termvar") " " (racket "wy50") ".")))
(provide charterm-wy50-keydec)
(define charterm-wy50-keydec
  (charterm-make-keydec 'wy50 charterm-wyse-wy50-keyset charterm-ascii-keyset))

(doc (subsubsub*section "TeleVideo Keydecs"))

(doc (defthing charterm-tvi925-keydec
               charterm-keydec?
               (para (tech "Keydec") " for " (tech "termvar") " " (racket "tvi925") ".")))
(provide charterm-tvi925-keydec)
(define charterm-tvi925-keydec
  (charterm-make-keydec 'tvi925 charterm-televideo-925-keyset charterm-ascii-keyset))

(doc (subsubsub*section "ASCII Keydecs"))

(doc (defthing charterm-ascii-keydec
               charterm-keydec?
               (para (tech "Keydec") " for " (tech "termvar") " " (racket "ascii") ".")))
(provide charterm-ascii-keydec)
(define charterm-ascii-keydec (charterm-make-keydec 'ascii charterm-ascii-keyset))

(doc (subsubsub*section "Default Keydecs"))

(doc (defthing charterm-ansi-keydec
               charterm-keydec?
               (para (tech "Keydec")
...
(doc
 (section (code "charterm") " Object")
 (para "The " (racket charterm) " object captures the state of a session with a particular terminal.")
 (para
  "A "
  (racket charterm)
...
(doc (defproc (charterm? (x any/c))
              boolean?
              (para "Predicate for whether or not " (var x) " is a " (racket charterm) ".")))
(provide charterm?)

(doc (defproc (charterm-termvar (ct charterm?)) (or/c #f string?))
     (para "Gets the " (tech "termvar") "."))
(provide charterm-termvar)

(doc (defproc (charterm-protocol (ct charterm?)) symbol?) (para "Gets the " (tech "protocol") "."))
(provide charterm-protocol)

(doc (defproc (charterm-keydec (ct charterm?)) symbol?) (para "Gets the " (tech "keydec") "."))
...
(define-struct charterm
               (tty in
                    ...
                    (screensize #:mutable))
  #:property prop:evt
  (struct-field-index evt))

(define (%charterm:protocol-unimplemented error-name ct)
  (error error-name "protocol unimplemented: ~S" (charterm-protocol ct)))

(define (%charterm:protocol-unreachable error-name ct)
  (error error-name "internal error: protocol unreachable: ~S" (charterm-protocol ct)))
  ...
  (case (system-type 'os)
    ...
    [else
     (let ([ostype (getenv "OSTYPE")])
       (if (and (string? ostype) (regexp-match? #rx"^(?i:freebsd|netbsd|openbsd)" ostype))
           "-f"
           "-F"))]))

(doc (defparam current-charterm
               ct
               (or/c #f charterm?)
...
(doc
 (defproc
  (open-charterm (#:tty tty (or/c #f path-string?) #f) (#:current? current? boolean? #t))
  charterm?
  (para "Returns an open "
        ...
        ", file "
        (filepath "/dev/tty")
        "), and setting raw mode and disabling echo (via the "
        (filepath "stty")
        " program).  If "
        (racket current?)
        " is true, the "
  ...
  (para
   "Note that this requires a Unix TTY to operate, so you normally
...
(define (open-charterm #:tty (tty #f) #:current? (current? #t))
  (let* ([tty (cleanse-path (or tty "/dev/tty"))] [tty-str (path->string tty)])
    (or
     (system* (find-executable-path "stty") %charterm:stty-minus-f-arg-string tty-str "raw" "-echo")
     (error 'open-charterm "stty ~S failed" tty-str))
    (with-handlers ([exn:fail? (lambda (e)
                                 (with-handlers ([exn:fail? void])
                                   (system* (find-executable-path "stty")
                                            %charterm:stty-minus-f-arg-string
                                            tty-str
                                            "sane"))
                                 (raise e))])
      (let*-values ([(in out) (open-input-output-file tty #:exists 'update)] [(buf-size) 2048])
            ...
            ([(termvar) (getenv "TERM")]
             ...
             [(protocol keydec)
              ...
              (cond
                ...
                ;; Default:
                [else (values #f #f)])]
             [(protocol keydec) (values (or protocol 'ansi) (or keydec charterm-ansi-keydec))])
          (letrec ([wrapping-evt (wrap-evt in (lambda (evt) ct))]
                   [ct (make-charterm tty-str ; tty
                                      ...
                                      (if (and (eq? protocol 'ansi)
                                          ...
                                          'control/stty/none
                                          'stty/none))])
            (and current? (current-charterm ct))
            ct))))))
...
(define (close-charterm #:charterm (ct (current-charterm)))
  (with-handlers ([exn:fail? void])
    (close-input-port (charterm-in ct)))
  (with-handlers ([exn:fail? void])
    (close-output-port (charterm-out ct)))
  ...
  (if (with-handlers ([exn:fail? (lambda (e) #f)])
        (system* (find-executable-path "stty")
                 ...
                 "cooked"
                 "echo"))
      (if (eq? ct (current-charterm)) (current-charterm #f) (void))
      (error 'close-charterm "stty failed")))
...
(define-syntax (with-charterm stx)
  (syntax-case stx ()
    [(_ BODY0 BODYn ...)
     #'(let ([ct #f])
         (dynamic-wind (lambda () (set! ct (open-charterm #:current? #t)))
                       (lambda ()
                         BODY0
                         BODYn ...)
                       (lambda ()
...
(doc
 (defproc
  (charterm-screen-size (#:charterm ct charterm? (current-charterm)))
  (values (or/c #f exact-nonnegative-integer?) (or/c #f exact-nonnegative-integer?))
  (para
   "Attempts to get the screen size, in character columns and rows.
It may do this through a control sequence or through the "
   (code "stty")
   " program.  If unable to get a value, then default of (80,24) is used.")
  (para
   "The current behavior in this version of "
   (code "charterm")
      ...
      [(control/stty/none)
       (let-values ([(cols rows) (%charterm:screen-size-via-control ct)])
         (if (and cols rows)
             (values cols rows)
             (begin
               (set-charterm-screensize! ct 'stty/none)
               (loop))))]
      [(stty/none)
       (let-values ([(cols rows) (%charterm:screen-size-via-stty ct)])
         (if (and cols rows)
             (values cols rows)
             (begin
               (set-charterm-screensize! ct 'none)
               (loop))))]
      [else (error 'charterm-screen-size "invalid screensize ~S" (charterm-screensize ct))])))

(define (%charterm:screen-size-via-control ct)
  (%charterm:protocol-case
   '%charterm:screen-size-via-control
   (charterm-protocol ct)
   ((ansi) (%charterm:write-bytes ct #"\e[18t")
           (cond
             [(%charterm:read-regexp-response ct #rx#"\e\\[8;([0-9]+);([0-9]+)t")
              =>
              (lambda (m)
             ...
             [else (values #f #f)]))
   ((wyse-wy50 televideo-925) (%charterm:protocol-unreachable '%charterm:screen-size-via-control
                                                              ct))))
  ...
  (let* ([stdout (open-output-bytes)]
    ...
    (if (eq? 'done-ok (proc 'status))
        (let-values ([(width height)
                      ...
                      ;; matches into 1-2.
                      (cond
                        [(regexp-match-positions #rx#"rows +([0-9]+);.*columns +([0-9]+)" bstr)
                         =>
                         (lambda (m)
                        ...
                        [(regexp-match-positions #rx#"columns +([0-9]+);.*rows +([0-9]+)" bstr)
                         =>
                         (lambda (m)
                           (values (%charterm:bytes-ascii->nonnegative-integer
                                    (subbytes bstr (caadr m) (cdadr m)))
                                   (%charterm:bytes-ascii->nonnegative-integer
                                    (subbytes bstr (caaddr m) (cdaddr m)))))]
                        [(regexp-match-positions #rx#" +([0-9]+) rows; +([0-9]+) columns;" bstr)
                         =>
                         (lambda (m)
                           (values (%charterm:bytes-ascii->nonnegative-integer
                                    (subbytes bstr (caaddr m) (cdaddr m)))
          ...
          (values (if (zero? width) #f width) (if (zero? height) #f height)))
        (values #f #f))))

(doc (section "Display Control"))

(define (%charterm:shift-buf ct)
  (let ([buf-start (charterm-buf-start ct)] [buf-end (charterm-buf-end ct)])
          ...
          [(eq? sync-result in)
           ...
           (let ([read-result (read-bytes-avail! (charterm-buf ct)
             ...
             (if (zero? read-result)
                 ...
                 (loop)
                 (begin
                   (set-charterm-buf-end! ct (+ (charterm-buf-end ct) read-result))
                   read-result)))]
          [else (error '%charterm:read-into-buf/timeout "*DEBUG* sync returned ~S" sync-result)])))))
          ...
          (begin
            (or
             (let ([buf (charterm-buf ct)]
               ...
               (cond
                 [(regexp-match-positions rx buf buf-start buf-end)
                  =>
                  (lambda (m)
                    ;; TODO: Audit and test some of this buffer
                    ;; code here and elsewhere.
                    (let ([match-start (caar m)] [match-end (cdar m)])
                      (if (= match-start buf-start)
                          (set-charterm-buf-start! ct match-end)
                          (if (= match-end buf-end)
                              (set-charterm-buf-end! ct match-start)
                              (begin
                                (bytes-copy! buf match-start buf match-end buf-end)
                                (set-charterm-buf-end! ct (+ match-start (- buf-end match-end)))))))

                    (map (lambda (pos) (subbytes buf (car pos) (cdr pos))) (cdr m)))]
                 [else #f]))
             (if (%charterm:read-into-buf/timeout ct timeout-seconds) (loop timeout-seconds) #f)))))))
...
(define (%charterm:bytes-ascii->nonnegative-integer bstr)
  (let ([bstr-len (bytes-length bstr)])
    (let loop ([i 0] [result 0])
      (if (= i bstr-len)
          result
          (let* ([b (bytes-ref bstr i)] [b-num (- b 48)])
            (if (<= 0 b-num 9)
                (loop (+ 1 i) (+ (* 10 result) b-num))
                (error '%charterm:bytes-ascii->nonnegative-integer "invalid byte ~S" b)))))))
...
(doc
 (defproc
  (charterm-display (#:charterm ct charterm? (current-charterm))
...
(define (charterm-display #:charterm (ct (current-charterm))
  ...
  (let ([out (charterm-out ct)]
    ...
    (let loop ([args args] [remaining-width (or width 0)])
      (if (null? args)
          (if (and pad (> remaining-width 0))
              ;; TODO: Get rid of this allocation.
              (begin
                (%charterm:write-bytes ct (make-bytes remaining-width 32))
                (void))
              (void))
          (let* ([arg (car args)]
                 [bytes
                  (cond
                    ...
                    [(string? arg)
                     (string->bytes/utf-8
                      ...
                      %charterm:err-byte
                      0
                      (if truncate (min (string-length arg) remaining-width) (string-length arg)))]
                    [(number? arg) (string->bytes/utf-8 (number->string arg) %charterm:err-byte)]
                    [else
                     (let ([arg (format "~A" arg)])
                       (string->bytes/utf-8 arg
                                            %charterm:err-byte
                                            0
                                            (if truncate
                                                (min (string-length arg) remaining-width)
                                                (string-length arg))))])]
                 [remaining-width (- remaining-width (bytes-length bytes))])
            (cond
              [(or (not truncate) (> remaining-width 0))
               (%charterm:write-bytes ct bytes)
               (loop (cdr args) remaining-width)]
              [(zero? remaining-width)
               (%charterm:write-bytes ct bytes)
               (void)]
              [else
               (%charterm:write-subbytes ct bytes 0 (+ (bytes-length bytes) remaining-width))
               (void)]))))))
  ...
  (let ([out (charterm-out ct)])
    (let loop ([args args])
      (if (null? args)
          ...
          (let ([arg (car args)])
            (cond
              [(bytes? arg) (write-bytes arg out)]
              [(string? arg) (write-string arg out)]
              [(integer? arg) (display (inexact->exact arg) out)]
              [(pair? arg)
               (loop (car arg))
               (loop (cdr arg))]
              [else (error '%charterm:send-code "don't know how to send ~S" arg)])
            (loop (cdr args)))))))
  ...
  (%charterm:protocol-case
   ...
   ((televideo-925)
    (if (and (= 1 x) (= 1 y))
        ...
        (begin
          (%charterm:write-bytes ct #"\e=")
          (%charterm:write-byte ct
                                (vector-ref %charterm:televideo-925-cursor-position-to-byte-vector y))
          (%charterm:write-byte ct
                                (vector-ref %charterm:televideo-925-cursor-position-to-byte-vector
                                            x)))))))
...
(doc (defproc* (((charterm-normal (#:charterm ct charterm? (current-charterm))) void?)
...
(define (charterm-normal #:charterm (ct (current-charterm)))
  (%charterm:protocol-case 'charterm-normal
                           (charterm-protocol ct)
...
(define (charterm-inverse #:charterm (ct (current-charterm)))
  (%charterm:protocol-case 'charterm-inverse
                           (charterm-protocol ct)
...
(define (charterm-underline #:charterm (ct (current-charterm)))
  (%charterm:protocol-case 'charterm-underline
                           (charterm-protocol ct)
...
(define (charterm-blink #:charterm (ct (current-charterm)))
  (%charterm:protocol-case 'charterm-blink
                           (charterm-protocol ct)
...
(define (charterm-bold #:charterm (ct (current-charterm)))
  (%charterm:protocol-case 'charterm-bold
                           (charterm-protocol ct)
...
(doc
 (defproc
  (charterm-clear-screen (#:charterm ct charterm? (current-charterm)))
  void?
  (para
...
(define (charterm-clear-screen #:charterm (ct (current-charterm)))
  ...
  (%charterm:protocol-case 'charterm-clear-screen
                           (charterm-protocol ct)
...
(define (charterm-clear-line #:charterm (ct (current-charterm)))
  (%charterm:protocol-case 'charterm:clear-line
                           (charterm-protocol ct)
...
(define (charterm-clear-line-left #:charterm (ct (current-charterm)))
  (%charterm:protocol-case
   ...
   ((televideo-925 wyse-wy50) (%charterm:unimplemented ct 'clearterm-clear-line-left))))

(provide charterm-clear-line-right)
(define (charterm-clear-line-right #:charterm (ct (current-charterm)))
  (%charterm:protocol-case 'charterm-clear-line-right
                           (charterm-protocol ct)
...
(define (charterm-insert-line (count 1) #:charterm (ct (current-charterm)))
  (if (integer? count)
      (cond
        [(= count 0) (void)]
        [(> count 0)
         (%charterm:protocol-case 'charterm-insert-line
                                  (charterm-protocol ct)
                                  ((ansi) (%charterm:send-code ct #"\e[" count "L"))
                                  ((wyse-wy50 televideo-925) (%charterm:write-bytes ct #"\eE")))]
        [else (error 'charterm-insert-line "invalid count: ~S" count)])
      (error 'charterm-insert-line "invalid count: ~S" count)))
      ...
      (cond
        ...
        [(> count 0)
         (%charterm:protocol-case
          ...
          (charterm-protocol ct)
          ((ansi) (%charterm:send-code ct #"\e[" count "M"))
           ...
           (if (= 1 count)
               ...
               (let ([bstr (make-bytes (* 2 count) 82)])
                 (let loop ([n (* 2 (- count 1))])
                   (bytes-set! bstr n 27)
                   (if (zero? n) (%charterm:write-bytes ct bstr) (loop (- n 2))))))))]
        [else (error 'charterm-delete-line "invalid count: ~S" count)])
      (error 'charterm-delete-line "invalid count: ~S" count)))

(doc (subsubsection "Misc. Output"))

(doc
 (defproc
  (charterm-bell (#:charterm ct charterm? (current-charterm)))
  void?
  (para
...
(doc
 ...
 (para "Normally you will get keyboard input using the " (racket charterm-read-key) " procedure."))

(doc
 (defproc
  (charterm-byte-ready? (#:charterm ct charterm? (current-charterm)))
  boolean?
  (para
...
(define (charterm-byte-ready? #:charterm (ct (current-charterm)))
  (or (> (charterm-buf-end ct) (charterm-buf-start ct)) (byte-ready? (charterm-in ct))))

(doc
 (defproc
  (charterm-read-key (#:charterm ct charterm? (current-charterm))
...
(define (charterm-read-key #:charterm (ct (current-charterm)) #:timeout (timeout #f))
  (%charterm:read-keyinfo-or-key 'charterm-read-key ct timeout #f))

(doc (defproc (charterm-read-keyinfo (#:charterm ct charterm? (current-charterm))
                                     (#:timeout timeout (or/c #f positive?) #f))
              charterm-keyinfo?
              (para "Like "
...
(define (charterm-read-keyinfo #:charterm (ct (current-charterm)) #:timeout (timeout #f))
...
(define (%charterm:read-keyinfo-or-key error-name ct timeout keyinfo?)
  ...
  (and (< (- (charterm-buf-size ct) (charterm-buf-start ct)) 10) (%charterm:shift-buf ct))
  (let ([buf (charterm-buf ct)]
        [buf-start (charterm-buf-start ct)]
        [buf-end (charterm-buf-end ct)]
    ...
    (if b1
        (or (let loop ([tree (charterm-keydec-primary-keytree keydec)]
              ...
              (cond
                [(hash-ref tree b #f)
                 ...
                 (lambda (code-or-subtree)
                   (cond
                     [(hash? code-or-subtree)
                      ;; We have more subtree to search.
                      (if (or (< probe-start buf-end)
                              (and (< buf-end buf-size) (%charterm:read-into-buf/timeout ct 0.5)))
                          ;; We have at least one more byte, so recurse.
                          (loop code-or-subtree (+ 1 probe-start) (bytes-ref buf probe-start))
                     ...
                     [(charterm-keyinfo? code-or-subtree)
                      ...
                      (begin
                        (set-charterm-buf-start! ct probe-start)
                        (if keyinfo? code-or-subtree (charterm-keyinfo-keycode code-or-subtree)))]
                     [else
                      (error error-name
            ...
            (cond
              [(hash-ref (charterm-keydec-secondary-keytree keydec) b1 #f)
               =>
               (lambda (keyinfo) (if keyinfo? keyinfo (charterm-keyinfo-keycode keyinfo)))]
              [else
               (if keyinfo?
                   ...
                   (make-charterm-keyinfo #f #f (list b1) "???" b1 (list b1))
                   (integer->char b1))]))
...
(define (%charterm:write-bytes ct bstr . rest-bstrs)
  ...
  (or (null? rest-bstrs) (for-each (lambda (bstr) (write-bytes bstr (charterm-out ct))) rest-bstrs)))
...
(define (%charterm:read-byte/timeout ct timeout)
  (let ([buf-start (charterm-buf-start ct)])
    (if (or (< buf-start (charterm-buf-end ct)) (%charterm:read-into-buf/timeout ct timeout))
...
(doc
 (section "References")
 (para "[" (deftech "ANSI X3.64") "] " (url "http://en.wikipedia.org/wiki/ANSI_escape_code"))
 (para "[" (deftech "ASCII") "] " (url "http://en.wikipedia.org/wiki/Ascii"))
 (para "["
       (deftech "ECMA-43")
       "] "
 ...
 (para
  ...
  "] "
  (hyperlink
   "http://www.mirrorservice.org/sites/www.bitsavers.org/pdf/televideo/Operators_Manual_Model_950_1981.pdf"
 ...
 (para "["
       ...
       (hyperlink "http://vt100.net/docs/vt100-tm/" (italic "VT100 Series Technical Manual"))
       ", 2nd Ed., 1980-09")
 (para "["
       (deftech "VT100-UG")
       "] "
       "Digital Equipment Corp., "
       (hyperlink "http://vt100.net/docs/vt100-ug/" (italic "VT100 User Guide"))
       ", 3rd Ed., 1981-06")
 (para "["
       (deftech "VT100-WP")
       "] "
       "Wikipedia, "
       (hyperlink "http://en.wikipedia.org/wiki/VT100" "VT100"))
 (para "["
       (deftech "WY-50-QRG")
       "] "
 ...
 (para "["
       (deftech "WY-60-UG")
       "] "
       (hyperlink "http://vt100.net/wyse/wy-60-ug/wy-60-ug.pdf" (italic "Wyse WY-60 User's Guide")))
 (para "["
       (deftech "wy60")
       "] "
       (hyperlink "http://code.google.com/p/wy60/" (code "wy60") " terminal emulator"))
 (para "["
       (deftech "XTerm-ctlseqs")
       "] "
       "Edward Moy, Stephen Gildea, Thomas Dickey, ``"
       (hyperlink "http://invisible-island.net/xterm/ctlseqs/ctlseqs.html" "Xterm Control Sequences")
       ",'' 2012")
 (para "[" (deftech "XTerm-Dickey") "] " (url "http://invisible-island.net/xterm/"))
 (para "["
       (deftech "XTerm-FAQ")
       "] "
       "Thomas E. Dickey, ``"
       (hyperlink "http://invisible-island.net/xterm/xterm.faq.html" "XTerm FAQ")
       ",'' dated 2012")
 (para "["
       (deftech "XTerm-WP")
       "] "
       "Wikipedia, "
       (hyperlink "http://en.wikipedia.org/wiki/Xterm" "xterm")))

(doc
 (section "Known Issues")
 (itemlist
  (item "Add a " (racket charterm-title) ".  Have it do sanitizing.")
  (item
   "Need to support ANSI alternate CSI for 8-bit terminals, even
before supporting 8-bit characters and multibyte.")
  (item
   "Only supports ASCII characters.  Adding UTF-8 support, for terminal emulators
...
;; !!! TODO: for NixOS, use "env stty". (Thanks to Till Höppner)
...
(doc
 ...
 (#:planet 5:0
  ...
  (itemlist
   (item
    "Incrementing major version because we're messing with "
    (code "stty")
    " locations and arguments, which is probably fine, but no
sense potentially breaking things for no reason.")
   (item "Now the "
         (code "stty")
         " program is found via the "
         (racket find-executable-path)
         " procedure, rather than at "
         (code "/bin/stty")
         ", for compatibility with Guix.  (Thanks to Sébastien Lerique.)")
   (item
    (code "stty")
    " should now use lowercase "
    (code "-f")
    " on FreeBSD, NetBSD, and OpenBSD.  No further testing
nor examination of docs.  (Thanks to Duncan Bayne.)")
   (item "Documentation tweaks.")))
 (#:planet 4:5
  #:date "2017-01-28"
  (itemlist
   (item
    "Documented about running within a terminal of some kind,
not DrRacket. (Thanks to Andrew Blinn.)")))
 (#:planet 4:4
           #:date "2016-03-10"
           (itemlist (item "Added support for another "
                           (code "stty -a")
                           " format, to "
                           (racket %charterm:screen-size-via-stty)
                           ".  (Thanks to Jay McCarthy.)")))
 (#:planet 4:3 #:date "2016-03-02" (itemlist (item "Tweaked info.rkt, filenames, documentation.")))
 (#:planet 4:2
           #:date "2016-02-26"
           (itemlist (item "Removed unimportant "
                           (filepath "test-charterm.rkt")
                           " file from package, for build server.")
                     (item "Documentation tweaks.")))
 (#:planet 4:1 #:date "2016-02-25" (itemlist (item "Fixed deps.")))
 (#:planet 4:0 #:date "2016-02-21" (itemlist (item "Moving from PLaneT to new package system.")))
 (#:planet 3:1
           #:date "2013-05-13"
           (itemlist (item "Now uses lowercase "
                           (code "-f")
                           " argument on MacOS X.  (Thanks to Jens Axel S\u00F8gaard for reporting.)")
                     (item "Documentation tweaks.")))
 (#:planet 3:0
           #:date "2012-07-13"
           (itemlist (item "Changed ``"
                           (code "ansi-ish")
                           "'' in identifiers to ``"
                           (code "ansi")
                           "'', hence the PLaneT major version number change.")
                     (item "Documentation tweaks.")
                     (item "Renamed package from ``" (code "charterm") "'' to ``CharTerm''.")))
 (#:planet 2:5
           #:date "2012-06-28"
           (itemlist (item "A " (racket charterm) " object is now a synchronizable event.")
                     (item "Documentation tweaks.")))
 (#:planet 2:4
  #:date "2012-06-25"
  (itemlist (item "Documentation fix for return type of " (racket charterm-read-keyinfo) ".")))
 (#:planet 2:3
  #:date "2012-06-25"
  (itemlist
   (item
    "Fixed problem determining screen size on some
XTerms.  (Thanks to Eli Barzilay for reporting.)")))
 (#:planet 2:2
  #:date "2012-06-25"
  (itemlist
   (item
    "Added another variation of encoding for XTerm arrow,
Home, and End keys.  (Thanks to Eli Barzilay.)")))
 (#:planet 2:1
           #:date "2012-06-24"
           (itemlist (item "Corrected PLaneT version number in " (racket require) " in an example.")))
 (#:planet 2:0
  #:date "2012-06-24"
  (itemlist
...
TeleVideo terminals, respectively, and compatibles.")
   ...
   (item
    "Demo is now in a separate file, mainly for convenience
...
previously-published example will need to have "
    (code ":1")
    " added to it anyway.")
   (item (racket charterm-screen-size) " now defaults to (80,24) when all else fails.")
...
adaptive for all terminals, with the shortcut for "
   ...
   (item "Documentation tweaks.")))
 (#:planet 1:0 #:date "2012-06-16" (itemlist (item "Initial version."))))
