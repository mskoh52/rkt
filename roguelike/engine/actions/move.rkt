        ...
        (match dest-actor
          [(cons dest-actor-key dest-actor)
           (let-values ([(updated-actor updated-dest-actor) (melee actor dest-actor)])
             (cleanup-state (nested-hash-set (nested-hash-set state 'actors actor-key updated-actor)
                                             'actors
                                             dest-actor-key
                                             updated-dest-actor)))])
        (if (equal? (hash-ref dest-terrain-def 'enter-cost) +inf.0)
            state
            (nested-hash-set state 'actors actor-key 'pos new-pos)))))

(module+ test
  (define (make-ground pos)
    (hash 'pos pos 'def (hash 'name "ground" 'glyph #\space 'enter-cost 0 'exit-cost 0)))
  (define state
    (hash 'actors
          (hash 'pc (hash 'pos p))
          'level
          (hash 'id
                0
                'terrain
                (map (lambda (y) (map (lambda (x) (make-ground `(,x . ,y))) (range 3))) (range 3)))))
